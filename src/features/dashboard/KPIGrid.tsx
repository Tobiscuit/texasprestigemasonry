    ...
    <>
      ...
      {stats.content && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-12">
          {[
            { label: 'Projects', value: stats.content.projects, href: '/dashboard/projects', color: '#f1c40f', icon: '🏗️' },
            { label: 'Services', value: stats.content.services, href: '/dashboard/services', color: '#3498db', icon: '⚙️' },
            { label: 'Published Posts', value: stats.content.publishedPosts, href: '/dashboard/posts', color: '#2ecc71', sub: `${stats.content.posts - stats.content.publishedPosts} drafts`, icon: '📝' },
            { label: 'Testimonials', value: stats.content.testimonials, href: '/dashboard/testimonials', color: '#e74c3c', sub: `${stats.content.featuredTestimonials} featured`, icon: '⭐' },
          ].map((item, i) => (
            <a
              key={i}
              href={item.href}
              className="flex items-center gap-3 p-3 md:p-4 rounded-2xl transition-all duration-200 group hover:-translate-y-0.5"
              style={{ backgroundColor: 'var(--staff-surface)', border: '1px solid var(--staff-border)' }}
            >
              <div className="text-2xl">{item.icon}</div>
              <div className="min-w-0">
                <div className="text-lg md:text-xl font-black" style={{ color: 'var(--staff-text)' }}>{item.value}</div>
                <div className="text-[10px] font-bold uppercase tracking-wider truncate" style={{ color: 'var(--staff-muted)' }}>
                  {item.label}
                  {item.sub && <span className="text-[var(--staff-muted)]/60 ml-1">· {item.sub}</span>}
                </div>
              </div>
            </a>
          ))}
